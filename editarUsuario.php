...
<body>
    <div class="container">
        <h1 class="title">Edite la información del usuario</h1>
        <br />
        <br />
        <!-- Envio a controlador con dato obtenido para validación e inserción -->
        <form action="hola.php" method="POST" id="form-editar-usuario">
            <div class="card">
                <label for="email">Editar:</label>
                <?php
                error_reporting(0);
                if (!empty($_GET["cedula"]) ) {
                    $cedula_usuario = $_GET['cedula'];
                    // Asignamos valor de cédula a input
                    ?>
                    <input type="text" name="cedula" id="cedula" value="<?php echo $cedula_usuario; ?>" readonly>
                    <?php
                }
                ?>
                <input type="text" name="nombre_editar" id="nombre_editar" placeholder="Ingrese su nombre">
                <input type="text" name="apellido_editar" id="apellido_editar" placeholder="Ingrese su apellido">
